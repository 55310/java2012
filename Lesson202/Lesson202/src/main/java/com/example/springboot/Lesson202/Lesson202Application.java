 ...
 * @Configuration : 클래스를 애플리케이션 컨텍스트에 대한 Bean 정의 소스 태그로 지정하는 역할
 * @EnableAutoConfiguration : Spring Boot에 클래스 경로 설정, 기타 Bean 및 다양한 속성 설정을 
 * 							  기반으로 Bean 추가를 시작하도록 지시하는 애노테이션
...
public class Lesson202Application {
	...
	/* @Bean으로 표시된 CommandLineRunner 메서드는 시작할 때 실행된다.
	 * 애플리케이션에서 생성했거나, Spring Boot에 의해 자동으로 추가된 모든 Bean을 검색하고,
	 * 정렬하여 출력한다.
	 * 
	 */
	@Bean
